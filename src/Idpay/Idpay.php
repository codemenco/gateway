...
namespace Codemenco\Gateway\Idpay;
...
//        throw new IdpayException( $payRequestResCode );
    ...
    public function validateCardNumber($cardNumber) {
        if (auth()->check()) {
            $userCards = BankAccount::where('user_id',Auth::id())->where('is_verified', ACTIVE)->where('is_active', ACTIVE)->pluck('swift');
            if (!empty($cardNumber)) {
                $return = false;
                foreach ($userCards as $userCard) {
                    $user4Num = substr($userCard , - 4);
                    $return4Num = substr($cardNumber , - 4);
                    if ($user4Num == $return4Num) {
                        $return = true;
                    }
                }
                if(!$return){
    ...
    protected function verifyPayment() {
        ...
        $this->validateCardNumber($response->payment->card_no);
